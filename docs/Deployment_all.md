...
OTA接口：
```
http://192.168.99.102:8002/xiaozhi/ota/
<!-- 用于设备的 ​​固件升级​​（通过 HTTP 下载新固件文件）。 -->
```

Websocket接口：
```
ws://192.168.99.102:8000/xiaozhi/v1/
<!-- 用于设备（如 ESP32）与服务器之间的 ​​实时双向通信​​（如指令下发、状态上报）。 -->
```

请你务必把以上两个接口地址写入到智控台中：他们将会影响websocket地址发放和自动升级功能。

1、使用超级管理员账号，登录智控台，在顶部菜单找到`参数管理`，找到参数编码是`server.websocket`，输入你的`Websocket接口`。192.168.99.102
